...
structure IsCovariantDerivativeOn
  ...
  leibniz {σ : Π x : M, V x} {g : M → 𝕜} {x}
    ...
    cov (g • σ) x = g x • cov σ x + (mfderiv' I g x).smulRight (σ x)
...
lemma congr_of_eqOn
  ...
  calc cov σ x
    _ = cov ((ψ : M → 𝕜) • σ) x := by
      simp [hcov.leibniz hσ hψ'.mdifferentiableAt, hψx, mfderiv', hψ'.mfderiv]
...
theorem smul_const (hcov : IsCovariantDerivativeOn F cov s)
  ...
  simpa [mfderiv'] using hcov.leibniz (g := fun _ ↦ a) hσ mdifferentiableAt_const
...
lemma finite_affine_combination {ι : Type*} {s : Finset ι}
  ...
  leibniz {σ g x} hσ hg hx := by
    calc ∑ i ∈ s, f i x • cov i (g • σ) x
      _ = ∑ i ∈ s, (g x • f i x • cov i σ x + f i x • (mfderiv' I g x).smulRight (σ x)) := by
          congr! 1 with i hi
          rw [(h i).leibniz hσ hg]
          simp [mfderiv']
          module
      _ = g x • ∑ i ∈ s, f i x • cov i σ x +
        (∑ i ∈ s, f i) x • (mfderiv' I g x).smulRight (σ x) := by
          rw [Finset.sum_add_distrib, Finset.smul_sum, Finset.sum_apply, Finset.sum_smul]
      _ = g x • ∑ i ∈ s, f i x • cov i σ x + (mfderiv' I g x).smulRight (σ x) := by rw [hf]; simp
